...
namespace GecolPro.Services
{
    public class Loggers : ILoggers
    {
        ...
        public Loggers()
        {
            ...
            CreateDirectoryIfNotExists(Path.Combine(baseDirectory, "Check"));
        ...
        {
            string filePath = Path.Combine(baseDirectory, category, $"{category.ToLower()}_{DateTime.Now:yyyyMMdd}.log");
        ...
        public async Task LogCheckAsync(string message)
        {
            await WriteLogAsync(message, "Check");
        }
    }
}
